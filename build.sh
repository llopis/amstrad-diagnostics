...
sjasmplus --nologo --msg=war -DLowerROMBuild=1 -DDandanatorSupport=1 --raw=build/AmstradDiagDandanator.rom --lst=build/AmstradDiagDandanator_list.txt src/Main.asm
...
cd build; zip AmstradDiag.zip *.rom ; cd ..
iDSK -n build/AmstradDiag.dsk
iDSK build/AmstradDiag.dsk -e 4000 -c 4000 -t 1 -i build/diag.bin
